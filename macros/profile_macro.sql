...
{%- macro read_information_schema_snowflake(db_name, include_schemas=[], exclude_schemas=[], include_tables=[], exclude_tables=[]) -%}
...
{%- macro create_new_schema_ssms(schema_name) -%}
...
{%- macro create_data_profile_table_ssms(db_name,schema_name,table_name)-%}
IF NOT EXISTS (
...
{% macro execute_raw_sql(query) %}
    {% do adapter.execute(query) %}
{% endmacro %}

{%- macro do_data_profile_ssms(information_schema_data, source_table_name, chunk_column, current_date_and_time) -%}
SELECT 
    '{{ information_schema_data[0] }}' AS database_name,
    '{{ information_schema_data[1] }}' AS schema_name,
    '{{ information_schema_data[2] }}' AS table_name,
    '{{ chunk_column[0] }}' AS column_name,
    '{{ chunk_column[1] }}' AS data_type,
    COUNT(*) AS row_count,
    COUNT({{ adapter.quote(chunk_column[0]) }}) AS not_null_count,
    COUNT(*) - COUNT({{ adapter.quote(chunk_column[0]) }}) AS null_count,
    ROUND((COUNT({{ adapter.quote(chunk_column[0]) }}) * 100.0) / COUNT(*), 2) AS not_null_percentage,
    ROUND((COUNT(CASE WHEN {{ adapter.quote(chunk_column[0]) }} IS NULL THEN 1 END) * 100.0) / COUNT(*), 2) AS null_percentage,
    COUNT(DISTINCT {{ adapter.quote(chunk_column[0]) }}) AS distinct_count,
    ROUND((COUNT(DISTINCT {{ adapter.quote(chunk_column[0]) }}) * 100 / COUNT(*)), 2) AS distinct_percent,
    CASE
        WHEN COUNT(DISTINCT {{ adapter.quote(chunk_column[0]) }}) = COUNT(*) THEN 'TRUE'
        ELSE 'FALSE'
    END AS is_unique,
    {% if chunk_column[1]|lower in ['int', 'bigint', 'decimal', 'numeric', 'real', 'double precision', 'smallint'] %}
    CAST(MIN({{ adapter.quote(chunk_column[0]) }}) AS VARCHAR) AS min,
    CAST(MAX({{ adapter.quote(chunk_column[0]) }}) AS VARCHAR) AS max,
    ROUND(AVG({{ adapter.quote(chunk_column[0]) }}), 2) AS avg,
    ROUND(STDEV({{ adapter.quote(chunk_column[0]) }}), 2) AS standard_deviation,
    {% else %}
    NULL AS min,
    NULL AS max,
    NULL AS avg,
    ...
    (SELECT
        STUFF((
            SELECT ', ' + CAST(value AS VARCHAR(255))
            FROM (
                SELECT DISTINCT {{ adapter.quote(chunk_column[0]) }} AS value
                FROM {{ source_table_name }}
                WHERE {{ adapter.quote(chunk_column[0]) }} IS NOT NULL
                ORDER BY value DESC
                OFFSET 0 ROWS FETCH NEXT 5 ROWS ONLY
            ) AS subquery
            FOR XML PATH('')), 1, 2, '') AS top_5_values),
    (SELECT
        STUFF((
            SELECT ', ' + CAST(value AS VARCHAR(255))
            FROM (
            ...
            FOR XML PATH('')), 1, 2, '') AS least_5_values)
FROM {{ source_table_name }}
{%- endmacro %}